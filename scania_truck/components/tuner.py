...
class ModelFinder:
    ...
    def get_trained_models(self, X_data, Y_data):
        try:
            ...
            x_train, y_train, x_test, y_test = (X_data[:, :-1], X_data[:, -1], Y_data[:, :-1], Y_data[:, -1])

            tuned_model_list = [(self.utils.get_tuned_model(model_name, x_train, y_train, x_test, y_test,)) for model_name in models_list]
