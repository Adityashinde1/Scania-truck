...
class DataValidation:
    ...
    def initiate_data_validation(self):
        ...
        try:
            ...
            logger.info(f"Schema train cols status is {schema_train_col_status} and schema test cols status is {schema_test_col_status}")

            logger.info("Validated dataset schema columns")

            (schema_train_num_cols_status, schema_test_num_cols_status) = self.validate_dataset_schema_for_numerical_datatype()

            logger.info(f"Schema train numerical cols status is {schema_train_num_cols_status} and schema test numerical cols status is {schema_test_num_cols_status}")

            logger.info("Validated dataset schema for numerical datatype")

            if (schema_train_num_cols_status is True and schema_test_num_cols_status is True and schema_train_col_status is True and schema_test_col_status is True):
